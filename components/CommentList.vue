            ...
            <ul class="comment-box">
                <li v-for="(item, index) in commentList" :key="item.id" class="comment-item">
                    <div class="comment-info">
                        ...
                        <div>
                            <p
                                class="comment-info-user"
                            >{{ (item.user_info && item.user_info.username) || '匿名评论' }}</p>
                            <p class="comment-info-timer">{{ item.created_at }}</p>
                        </div>
                    </div>
                    <div class="comment-item-content" v-html="mdRender(item.content)"></div>

                    <ul v-if="item.replys" class="reply-list">
                        <template v-for="reply in item.replys">
                            <li v-if="reply.status == 1" :key="reply.id" class="reply-item">
                                <div class="comment-info">
                                    <div class="comment-info-avatar">
                                    ...
                                    <div>
                                        <p
                                            class="comment-info-user"
                                        >{{ (reply.user_info && reply.user_info.username) || '匿名回复' }}</p>
                    ...
                    <div class="reply-create">
                        ...
                        <div v-if="item.is_show_reply">
                            <From
                                from-type="reply"
                                :reply="{ comment_id: item.id, reply_user_id: item.user_id }"
                                @on-success="showReplyUser"
                            />
                        </div>
                    </div>
                </li>
