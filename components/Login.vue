<template>
    <div>
        <div class="login">
            ...
            <h2 class="login-header">{{ isLogin ? '登录' : '注册' }}</h2>
            ...
            <div
                class="login-register-tips"
                @click="isLogin = !isLogin"
            >{{ isLogin ? '未有账号？点击注册！' : '已有账号？点击登录！' }}</div>
            <div class="login-btn">
                <button
                    :disabled="isDisabled"
                ...
                <button class="login-btn-default" @click="onSuccess(false)">匿名评论</button>
            </div>
        </div>
    </div>
</template>
